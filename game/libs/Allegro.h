...
int Allegro(camera *cam, ALLEGRO_DISPLAY *display){	
	...
	if(/*!background || !bomb ||*/ !bexiga || !robot 
	...
	skill						bexiga 							= newSkill(bexiga, 10, 50);
	...
	character 			robot					 			= newCharacter(robot);
	...
	while(TRUE){
		...
		if(coordenadas[0] < aux_x + 50 && coordenadas[0] > aux_x - 50 
			...
			drawTarget(targetSelected, coordenadas[0], coordenadas[1], 0);
		}
